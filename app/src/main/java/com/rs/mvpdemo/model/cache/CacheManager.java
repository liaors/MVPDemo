package com.rs.mvpdemo.model.cache;
...
public final class CacheManager {
    ...
    private CacheManager() {
        File diskCacheDir = getDiskCacheDir(CommonUtils.appContext, CACHE_DIR);
        ...
        if (diskCacheDir.getUsableSpace() > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir,
                        getAppVersion(CommonUtils.appContext), 1/*一个key对应多少个文件*/, DISK_CACHE_SIZE);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
